...
PACKAGE_VERSION=${1:-0.1.9}
PACKAGE_URL=https://github.com/deepmind/tree
PACKAGE_DIR="tree/"

# Install build dependencies
yum install -y gcc gcc-c++ make cmake3 git wget xz zlib-devel openssl-devel \
  bzip2-devel libffi-devel python3.12 python3.12-devel python3.12-pip python3.12-wheel python3.12-setuptools
...
python3.12 -m pip install --upgrade pip wheel pytest absl-py attrs numpy==2.2.6 wrapt
...
python3.12 setup.py build_ext --build-temp=build_temp --inplace -j$(nproc) || true
