...
if ! ./gradlew client:jar; then
       echo "------------------$PACKAGE_NAME:Client Build_fails---------------------"
       echo "$PACKAGE_VERSION $PACKAGE_NAME"
       echo "$PACKAGE_NAME  | $PACKAGE_VERSION | $OS_NAME | GitHub | Fail |  Build_Fails"
       exit 1
fi

if ! ./gradlew client:test -PignoreFailures=true ; then
...
# Please Note : 2 test cases have been skipped due to failures, they are in parity with x86.