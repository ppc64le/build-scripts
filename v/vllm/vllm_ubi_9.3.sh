...
export SETUPTOOLS_SCM_PRETEND_VERSION=2.6.0
...
# Apply the patch
echo "------------------------Applying patch-------------------"
wget https://raw.githubusercontent.com/ppc64le/build-scripts/refs/heads/master/t/torchaudio/torchaudio_v2.6.0.patch
...
yum install -y gmp-devel freetype-devel
...
pip3.12 install   scikit-build setuptools wheel
pip3.12 install -e . 
...
yum install -y meson libffi-devel zlib-devel \
    libtiff-devel freetype-devel make cmake automake autoconf procps-ng \
    python3.12 python3.12-devel python3.12-pip
...
yum install -y gcc-toolset-13 make libtool cmake xz zlib-devel libffi-devel libevent-devel libjpeg-turbo-devel python python-devel
# export PATH=/opt/rh/gcc-toolset-13/root/usr/bin:$PATH
...
pip3.12 install  absl-py attr numpy wrapt attrs
...
cd  gmpy
git checkout v2.2.1
pip3.12 --verbose install --editable .[docs,tests]
pip3.12 install .
echo "------------------------installed gmpy2-------------------------"

cd $CURRENT_DIR
echo "------------------------installing grpcio-------------------------"
yum install -y python3 python3-devel python3-pip   git gcc-toolset-13 gcc-toolset-13-gcc-c++
git clone https://github.com/grpc/grpc.git
cd grpc/
git checkout v1.71.0
git submodule update --init --recursive
pip3.12 install coverage  
...
yum install -y  python-devel java-11-openjdk java-11-openjdk-devel gzip tar make xz cmake yum-utils  openblas-devel bzip2-devel bzip2 zip unzip libffi-devel zlib-devel autoconf automake libtool cargo pkgconf-pkg-config.ppc64le info.ppc64le fontconfig.ppc64le fontconfig-devel.ppc64le sqlite-devel
...
WORKING_DIR="$(pwd)"
yum install -y   bzip2-devel libffi-devel zlib-devel cmake python3.11 python3.11-devel python3.11-pip
...
cd $CURRENT_DIR
echo "------------------------installing zfp-------------------------"
yum install -y  gcc-toolset-13-gcc-c++ gcc-toolset-13-gcc-gfortran  make python python-devel python-pip  cmake 
# export PATH=/opt/rh/gcc-toolset-13/root/usr/bin:$PATH
export LD_LIBRARY_PATH=/opt/rh/gcc-toolset-13/root/usr/lib64:$LD_LIBRARY_PATH
...
if [[ "$MAJOR" -gt 3 ]] || { [[ "$MAJOR" -eq 3 ]] && [[ "$MINOR" -ge 12 ]]; }; then
    ...
    pip3.12 install cython==0.29.36 numpy==1.23.5 
fi
...
yum install -y python3.12 python3.12-devel python3.12-pip gcc-toolset-13 
# export PATH=/opt/rh/gcc-toolset-13/root/usr/bin:$PATH
# export LD_LIBRARY_PATH=/opt/rh/gcc-toolset-13/root/usr/lib64:$LD_LIBRARY_PATH
...
yum install -y  bzip2-devel libffi-devel zlib-devel cmake
#clone repository
...
yum install -y  python-devel bzip2-devel libffi-devel xz zlib-devel cmake openblas-devel
#clone repository
git clone https://github.com/pydata/numexpr.git
cd  numexpr
git checkout v2.8.4
pip3.12 install 
pip3.12 install numpy==1.26.3
pip3.12 install -e .
python3.12 setup.py install
...
cd $CURRENT_DIR
echo "------------------------installing opencv_python_headless-------------------------"
yum install -y  gcc-toolset-13 gcc-toolset-13-gcc gcc-toolset-13-gcc-c++ gcc-toolset-13-gcc-gfortran gcc-toolset-13-binutils gcc-toolset-13-binutils-devel python python-pip python-devel git  make cmake pkgconfig autoconf automake libtool zlib-devel freetype-devel gmp-devel    
# export PATH=/opt/rh/gcc-toolset-13/root/usr/bin:$PATH
# export LD_LIBRARY_PATH=/opt/rh/gcc-toolset-13/root/usr/lib64:$LD_LIBRARY_PATH
...
git checkout 5.2.0
pip3.12 install 
pip3.12 install "chardet<5,>=3.0.2" --force-reinstall
...
yum install -y  gcc-toolset-13-gcc gcc-toolset-13-gcc-c++ gcc-toolset-13-gcc-gfortran \
    cmake   bzip2-devel glibc-static libstdc++-static libffi-devel \
    zlib-devel python-devel python3.12-pip pkg-config automake autoconf libtool
...
pip3.12 install cython==3.0.12 packaging==24.2 pathspec==0.12.1 scikit-build-core==0.11.1  ninja==1.11.1.4 build  -asyncio -timeout
pip3.12 install -e .
...
yum install -y  gcc-toolset-13-gcc gcc-toolset-13-gcc-c++ cmake  bzip2-devel glibc-static libstdc++-static libffi-devel zlib-devel pkg-config automake autoconf libtool
...
yum install -y python3-pip python3 python python3-devel 
...
pip3.12 install  hypothesis build meson pybind11 meson-python
echo "Python environment set up."
 
# Install package dependencies
pip3.12 install 'numpy<2' fontTools setuptools-scm contourpy kiwisolver python-dateutil cycler pyparsing pillow certifi
...
yum install -y libtool  xz zlib-devel  bzip2-devel libffi-devel libevent-devel  patch gcc-toolset-13  pkg-config  gmp-devel  freetype-devel
...
pip3.12 install -v -e . --no-build-isolation
...
yum install -y kmod
python3.12 -m pip install setuptools-rust maturin setuptools_scm
export LD_LIBRARY_PATH=/protobuf/local/libprotobuf/lib64:$LD_LIBRARY_PATH
