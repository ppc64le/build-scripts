...
if [ -n "$VAULT_REDIRECT_INTERFACE" ]; then
    export VAULT_REDIRECT_ADDR=$(get_addr $VAULT_REDIRECT_INTERFACE ${VAULT_REDIRECT_ADDR:-"http://127.0.0.1:8200"})
    echo "Using $VAULT_REDIRECT_INTERFACE for VAULT_REDIRECT_ADDR: $VAULT_REDIRECT_ADDR"
fi
if [ -n "$VAULT_CLUSTER_INTERFACE" ]; then
    export VAULT_CLUSTER_ADDR=$(get_addr $VAULT_CLUSTER_INTERFACE ${VAULT_CLUSTER_ADDR:-"https://127.0.0.1:8201"})
...
if [ "$1" = 'server' ]; then
    ...
    set -- vault server \
        ...
        -dev-listen-address="${VAULT_DEV_LISTEN_ADDRESS:-"127.0.0.1:8200"}" \
        "$@"
elif [ "$1" = 'version' ]; then
