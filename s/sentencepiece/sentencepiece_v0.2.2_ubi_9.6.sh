...
yum install -y make libtool git wget tar xz zlib-devel openssl-devel bzip2-devel libffi-devel libevent-devel patch python python-devel ninja-build gcc-toolset-13 gcc gcc-c++ pkg-config cmake
...
python3 -m pip install --upgrade pip wheel "setuptools<80" ninja packaging pytest pybind11

export PYBIND11_INCLUDES=$(python3 -c "import pybind11; print(pybind11.get_include())")
export CFLAGS="${CFLAGS} -I${PYBIND11_INCLUDES}"
export CXXFLAGS="${CXXFLAGS} -I${PYBIND11_INCLUDES}"
