...
echo "Cloning the xgboost repository..."
mkdir -p output
...
# Only add the packages line if it's not already present
grep -q '^\s*packages\s*=\s*\["xgboost/"\]' pyproject.toml || sed -i '/^\[tool.hatch.build.targets.wheel\]/a packages = ["xgboost/"]' pyproject.toml
