...
TF_WHEEL_OUTPUT=/tmp/tf_wheel_output
mkdir -p $TF_WHEEL_OUTPUT
./bazel-bin/tensorflow/tools/pip_package/build_pip_package $TF_WHEEL_OUTPUT
mkdir -p $SRC_DIR/tensorflow_pkg
find $TF_WHEEL_OUTPUT -iname "*.whl" -exec cp {} $SRC_DIR/tensorflow_pkg/ \;
find $TF_WHEEL_OUTPUT -iname "*.whl" -exec cp {} $CURRENT_DIR/ \;
echo "----------wheel created----------------"

echo "---------------------------------Running tests----------------------------------------------"
TEST_LOG=$(mktemp)
bazel --bazelrc=$BAZEL_RC_DIR/tensorflow.bazelrc test -k \
  --jobs=HOST_CPUS*0.25 \
  --local_cpu_resources=HOST_CPUS*0.25 \
  --local_ram_resources=HOST_RAM*0.50 \
  --define=tflite_with_xnnpack=false \
  --action_env=PATH=/opt/rh/gcc-toolset-13/root/usr/bin:$PATH \
  --action_env=CC=/opt/rh/gcc-toolset-13/root/usr/bin/gcc \
  --action_env=CXX=/opt/rh/gcc-toolset-13/root/usr/bin/g++ \
  //tensorflow/tools/pip_package/... 2>&1 | tee "$TEST_LOG"
