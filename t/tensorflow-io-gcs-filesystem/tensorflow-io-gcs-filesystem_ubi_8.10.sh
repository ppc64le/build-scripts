...
yum install -y libffi-devel freetype-devel procps-ng meson gcc-gfortran libomp-devel zip unzip sqlite sqlite-devel libxcrypt rsync
...
make
make install
ln -sf /usr/local/bin/patchelf /usr/bin/patchelf
...
cd $CURRENT_DIR/io
python3.11 -m pip install "setuptools<80"
...
sed -i 's|http://github.com/libexpat/libexpat/archive/R_2_2_6.tar.gz|https://github.com/libexpat/libexpat/archive/R_2_2_6.tar.gz|' WORKSPACE
...
python3.11 setup.py bdist_wheel --data bazel-bin --project tensorflow-io-gcs-filesystem --dist-dir $CURRENT_DIR
