...
# -----------------------------------------------------------------------------
#
# Package	    : ttl-lru-cache
# Version	    : 0.0.2
# Source repo	    : https://github.com/serby/ttl-lru-cache
# Tested on	    : UBI 9.3
# Language          : Node
# Travis-Check      : True
...
PACKAGE_NAME=ttl-lru-cache
PACKAGE_VERSION=${1:-0.0.2}
PACKAGE_URL=https://github.com/serby/ttl-lru-cache
...
if ! npm install mocha@2.3.4; then
    echo "------------------$PACKAGE_NAME:install_fails-------------------------------------"
    echo "$PACKAGE_URL $PACKAGE_NAME"
    echo "$PACKAGE_NAME  |  $PACKAGE_URL | $PACKAGE_VERSION | GitHub | Fail |  Install_Fails"
    exit 1
