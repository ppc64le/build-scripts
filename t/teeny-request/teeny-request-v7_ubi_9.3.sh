...
# Version	    : v7.2.0
# Source repo	    : https://github.com/googleapis/teeny-request
# Tested on	    : UBI 9.3
# Language          : Node
# Travis-Check      : True
...
PACKAGE_NAME=teeny-request
PACKAGE_VERSION=${1:-v7.2.0}
PACKAGE_URL=https://github.com/googleapis/teeny-request
...
if ! npm install typescript@5.1.6; then
    echo "------------------$PACKAGE_NAME:install_fails-------------------------------------"
    echo "$PACKAGE_URL $PACKAGE_NAME"
    echo "$PACKAGE_NAME  |  $PACKAGE_URL | $PACKAGE_VERSION | GitHub | Fail |  Install_Fails"
    exit 1
