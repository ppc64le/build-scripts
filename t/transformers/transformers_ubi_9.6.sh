...
# Version       : v4.52.4
# Source repo   : https://github.com/huggingface/transformers.git
# Tested on     : UBI 9.6
# Language      : Python
# Ci-Check  : True
# Script License: Apache License 2.0
# Maintainer    : Rushikesh Sathe<Rushikesh.Sathe2ibm.com>
#
...
python3.12 -m pip install sentencepiece==0.1.97 parameterized datasets==4.0.0 setuptools wheel pytest timeout_decorator evaluate GitPython ruff psutil packaging pyyaml
python3.12 -m pip install --prefer-binary llvmlite numpy scipy pillow numba==0.62.0.dev0 c-ares thrift-cpp snappy libprotobuf orc utf8proc re2 openblas grpcio ml-dtypes h5py hdf5==1.12.3 torch==2.6.0 --extra-index-url=https://wheels.developerfirst.ibm.com/ppc64le/linux

python3.12 -m pip install accelerate>=1.12.0

export VENV_SITE="$VENV_DIR/lib/python$PYTHON_VERSION/site-packages"
export LD_LIBRARY_PATH="$VENV_SITE/hdf5/lib:$LD_LIBRARY_PATH"
export LD_LIBRARY_PATH="$VENV_SITE/snappy/lib:$LD_LIBRARY_PATH"
export LD_LIBRARY_PATH="$VENV_SITE/libprotobuf/lib64:$LD_LIBRARY_PATH"
export LD_LIBRARY_PATH="$VENV_SITE/thriftcpp/lib:$LD_LIBRARY_PATH"
export LD_LIBRARY_PATH="$VENV_SITE/orc/lib:$LD_LIBRARY_PATH"
export LD_LIBRARY_PATH="$VENV_SITE/utf8proc/lib:$LD_LIBRARY_PATH"
export LD_LIBRARY_PATH="$VENV_SITE/re2/lib:$LD_LIBRARY_PATH"
export LD_LIBRARY_PATH="$VENV_SITE/openblas/lib:$LD_LIBRARY_PATH"
echo "-----------------cloning transformers package---------------------"
...
SRC_DIR=$(pwd)
wget https://raw.githubusercontent.com/ppc64le/build-scripts/refs/heads/master/t/transformers/transformers_4.52.4.patch
git apply transformers_4.52.4.patch
...
#pytest skipping test that are in parity with x86, also skipping jax/jaxlib related tests 

if ! pytest --ignore=templates --ignore=examples --ignore=docs --ignore=i18n --ignore=tests/jax --ignore=tests/utils --ignore=tests/pipelines --ignore=tests/models --ignore=tests/extended --ignore=tests/trainer --ignore=tests/generation --ignore=tests/tokenization --ignore=tests/tensor_parallel --ignore=tests/test_pipeline_mixin.py --ignore=tests/repo_utils/modular -k "not test_evaluation_with_keys_to_drop" -q --disable-warnings
then
