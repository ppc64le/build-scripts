...
git apply $SCRIPT_DIR/helm-unittest_v0.2.11.patch
make install
helm plugin list
...
# Run Tests
#######################################
make test WEBASSETS_SKIP_BUILD=1 RDPCLIENT_TAG=desktop_access_rdp


echo "Test output should be:'DONE 20572 tests, 213 skipped' which is in parity with intel "
echo "Skipping TestSessionAuditLog, on ppc64le: X11 forwarding not supported in this environment"
