...
#./gradlew -a :junit-jupiter-api:build | tee $LOGS_DIRECTORY/$PKG_NAME-$PKG_VERSION.txt
...
./gradlew -a :junit-jupiter-api:build | tee $LOGS_DIRECTORY/$PKG_NAME-$PKG_VERSION_LATEST.txt

# clone, build and test master
#cd $LOCAL_DIRECTORY
#git clone $REPOSITORY $PKG_NAME-master
#cd $PKG_NAME-master/
#./gradlew -a :junit-jupiter-api:build | tee $LOGS_DIRECTORY/$PKG_NAME.txt
