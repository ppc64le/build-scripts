# -----------------------------------------------------------------------------
#
# Package	: google-api-client-java6
# Version	: v1.27.0
# Source repo	: https://github.com/googleapis/google-api-java-client.git
# Tested on	: UBI 8.5
# Language	: JAVA
# Travis-Check  : True
# Script License: Apache License, Version 2 or later
# Maintainer	: Sapna Shukla <Sapna.Shukla@ibm.com>
#
# Disclaimer: This script has been tested in root mode on given
# ==========  platform using the mentioned version of the package.
#             It may not work as expected with newer versions of the
#             package and/or distribution. In such case, please
#             contact "Maintainer" of this script.
#
# ----------------------------------------------------------------------------

HOME_DIR=`pwd`
PACKAGE_NAME=google-api-java-client/google-api-client-java6
PACKAGE_VERSION=${1:-v1.27.0}
PACKAGE_URL=https://github.com/googleapis/google-api-java-client.git

mkdir -p output

OS_NAME=$(cat /etc/os-release | grep ^PRETTY_NAME | cut -d= -f2)

yum install git maven -y

#clone the repository
git clone $PACKAGE_URL

export WORK_DIR=$HOME_DIR/$PACKAGE_NAME
cd $WORK_DIR
git checkout $PACKAGE_VERSION

# ----------------------------------------------------------------------------
# No test available, therefore just kept mvn install and no mvn test:
#
# Snippet of mvn test:
#
# [INFO] --- maven-compiler-plugin:2.3.2:testCompile (default-testCompile) @ google-api-client-java6 ---
# [INFO] No sources to compile
# [INFO]
# [INFO] --- maven-surefire-plugin:2.19.1:test (default-test) @ google-api-client-java6 ---
# [INFO] No tests to run.
# [INFO] ------------------------------------------------------------------------
# [INFO] BUILD SUCCESS
# [INFO] ------------------------------------------------------------------------
# [INFO] Total time: 4.842 s
# [INFO] Finished at: 2022-01-06T08:39:38Z
# [INFO] ------------------------------------------------------------------------
# ----------------------------------------------------------------------------

if ! mvn clean install; then
	echo "------------------$PACKAGE_NAME:install_fails-------------------------------------"
	echo "$PACKAGE_VERSION $PACKAGE_NAME" > $HOME_DIR/output/install_fails
	echo "$PACKAGE_NAME  |  $PACKAGE_VERSION | $OS_NAME | GitHub | Fail |  Install_Fails" > $HOME_DIR/output/version_tracker
	exit 1
else
	echo "------------------$PACKAGE_NAME:install_&_test_not_available-------------------------"
	echo "$PACKAGE_URL $PACKAGE_NAME" > $HOME_DIR/output/test_success 
	echo "$PACKAGE_NAME  | $PACKAGE_VERSION | $OS_NAME | GitHub  | Pass |  Install_Success" > $HOME_DIR/output/version_tracker
	find -name *.jar >> $HOME_DIR/output/post_build_jars.txt
	echo "------------PATH of .JAR created for $WORK_DIR can be checked in text file:$HOME_DIR/output/post_build_jars.txt -----------"
	exit 0
fi


