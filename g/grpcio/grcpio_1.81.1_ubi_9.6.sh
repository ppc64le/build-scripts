...
dnf install -y python3.12 python3.12-devel python3.12-pip openssl openssl-devel git gcc-toolset-13 gcc-toolset-13-gcc-c++

# Clone the grpc package.
PACKAGE_NAME=grpc
PACKAGE_VERSION=${1:-v1.81.1}
...
[[ "$(printf "%s\n" "$PACKAGE_VERSION" v1.62.3 | sort -V | head -n1)" == "$PACKAGE_VERSION" ]] && python3.12 -m pip install --force-reinstall Cython==0.29.37
