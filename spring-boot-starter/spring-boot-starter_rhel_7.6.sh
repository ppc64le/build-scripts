...
# variables
PKG_NAME="spring-boot-starter"
PKG_VERSION=2.1.12
PKG_VERSION_LATEST=2.2.6
LOGS_DIRECTORY=/logs
...
cd spring-boot-project/spring-boot-starters/spring-boot-starter/
mvn install | tee $LOGS_DIRECTORY/$PKG_NAME-$PKG_VERSION.txt
# build all starters
cd ..
mvn install | tee $LOGS_DIRECTORY/$PKG_NAME-$PKG_VERSION-x.txt
...
# build all starters
cd ..
mvn install | tee $LOGS_DIRECTORY/$PKG_NAME-$PKG_VERSION_LATEST-x.txt
