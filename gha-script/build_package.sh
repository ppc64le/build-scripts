...
if [[ "$TESTED_ON" == UBI:9* || "$TESTED_ON" == UBI9* ]]; then
    docker pull registry.access.redhat.com/ubi9/ubi:9.6
    docker_image="registry.access.redhat.com/ubi9/ubi:9.6"
    if [[ "$NON_ROOT_BUILD" == "true" ]]; then
        docker_build_non_root "registry.access.redhat.com/ubi9/ubi:9.6"
    fi
else
