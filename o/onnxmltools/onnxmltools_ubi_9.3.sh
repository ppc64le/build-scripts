...
yum install -y git make libtool wget gcc-toolset-13-gcc gcc-toolset-13-gcc-c++ gcc-toolset-13-gcc-gfortran libevent-devel zlib-devel openssl-devel clang python3.12 python3.12-devel python3.12-pip  xz bzip2-devel libffi-devel patch ninja-build
...
echo "-------build and install openblas-----------"
...
cd $WORK_DIR
pip3.12 install --upgrade pip setuptools wheel ninja packaging tox build mypy stubs packaging pathspec pluggy trove-classifiers pytest
...
/usr/bin/python3 -m pip3.12 install packaging wheel
...
echo "-------build onnxmtools installed-----------"
#Build onnxmltools
git clone $PACKAGE_URL
cd $PACKAGE_DIR
git checkout $PACKAGE_VERSION

pip3.12 install pyspark coremltools h2o
