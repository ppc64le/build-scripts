...
RUN yum update -y && yum install -y tar gzip shadow-utils openssl-devel which && yum clean all
...
COPY * $TEMP_DIR/
COPY opensearch-min-2.18.0-SNAPSHOT-linux-ppc64le.tar.gz $TEMP_DIR/
COPY security-config $TEMP_DIR/security-config

RUN ls -l $TEMP_DIR &&  \
    tar -xzpf $TEMP_DIR/opensearch-min-2.18.0-SNAPSHOT-linux-ppc64le.tar.gz -C $OPENSEARCH_HOME --strip-components=1 && \
    ...
    cp -v $TEMP_DIR/opensearch-docker-entrypoint-2.x.sh $OPENSEARCH_HOME/opensearch-docker-entrypoint.sh && \
    cp -v $TEMP_DIR/opensearch-onetime-setup.sh $OPENSEARCH_HOME/ && \
    cp -v $TEMP_DIR/log4j2.properties $TEMP_DIR/opensearch.yml $TEMP_DIR/root-ca.pem $TEMP_DIR/securityadmin.sh $TEMP_DIR/kirk-key.pem $TEMP_DIR/kirk.pem $TEMP_DIR/esnode-key.pem $TEMP_DIR/esnode.pem $OPENSEARCH_PATH_CONF/ && \
    mkdir $OPENSEARCH_PATH_CONF/opensearch-performance-analyzer && \
    cp -v $TEMP_DIR/plugin-stats-metadata $OPENSEARCH_PATH_CONF/opensearch-performance-analyzer && \
    mkdir $OPENSEARCH_PATH_CONF/opensearch-security && \
    cp -r $TEMP_DIR/security-config/* $OPENSEARCH_PATH_CONF/opensearch-security/ && \
