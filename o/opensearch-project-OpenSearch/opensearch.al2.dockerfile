...
# This dockerfile generates an ubi9 image containing an OpenSearch installation (1.x Only).
...
COPY * $TEMP_DIR/
COPY opensearch-min-2.19.2-SNAPSHOT-linux-ppc64le.tar.gz $TEMP_DIR/
COPY security-config $TEMP_DIR/security-config

RUN ls -l $TEMP_DIR &&  \
    tar -xzpf $TEMP_DIR/opensearch-min-2.19.2-SNAPSHOT-linux-ppc64le.tar.gz -C $OPENSEARCH_HOME --strip-components=1 && \
