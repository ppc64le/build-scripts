...
if ! ./gradlew assemble -x :distribution:docker:buildDockerImage -x :distribution:docker:buildArm64DockerImage -x :distribution:docker:buildPpc64leDockerImage -x :distribution:docker:buildRiscv64DockerImage -x :distribution:docker:buildS390xDockerImage; then
        echo "------------------$PACKAGE_NAME:Build_fails---------------------"
        echo "$PACKAGE_VERSION $PACKAGE_NAME"
        echo "$PACKAGE_NAME  | $PACKAGE_VERSION | $OS_NAME | GitHub | Fail |  Build_Fails"
        exit 1
#Commenting the command for gradlew check as the entire testsuite takes around 3-4 hours to complete.		
# elif ! ./gradlew check --continue -x server:test -x server:internalClusterTest -x plugins:repository-s3:testRepositoryCreds -x plugins:repository-s3:s3ThirdPartyTest   -x plugins:repository-s3:yamlRestTestECS   -x plugins:repository-s3:test   -x plugins:repository-s3:yamlRestTestEKS   -x plugins:repository-s3:yamlRestTestMinio   -x plugins:repository-s3:yamlRestTest   -x plugins:ingestion-kafka:internalClusterTest   -x plugins:ingestion-kinesis:internalClusterTest   -x distribution:packages:no-jdk-arm64-rpm:checkExtraction   -x distribution:packages:no-jdk-arm64-rpm:checkLicense   -x distribution:packages:no-jdk-arm64-rpm:checkNotice -x qa:smoke-test-multinode:integTest  --max-workers=2   --no-daemon   -Dorg.gradle.jvmargs="-Xmx2g --enable-native-access=ALL-UNNAMED" ; then
