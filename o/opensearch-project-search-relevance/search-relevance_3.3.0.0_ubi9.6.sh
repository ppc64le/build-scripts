...
# Version          : 3.3.0.0
# Source repo      : https://github.com/opensearch-project/search-relevance
# Tested on        : UBI:9.6
# Language         : Java
# Ci-Check     : True
...
PACKAGE_NAME=search-relevance
PACKAGE_URL=https://github.com/opensearch-project/search-relevance
...
# --------
ret=0
./gradlew build -PcustomDistributionUrl=$wdir/OpenSearch/distribution/archives/linux-ppc64le-tar/build/distributions/opensearch-min-$OPENSEARCH_VERSION-SNAPSHOT-linux-ppc64le.tar.gz   -Dbuild.snapshot=false || ret=$?
if [ $ret -ne 0 ]; then
        set +ex
...
# -----------------
ret=0
./gradlew test -PcustomDistributionUrl=$wdir/OpenSearch/distribution/archives/linux-ppc64le-tar/build/distributions/opensearch-min-$OPENSEARCH_VERSION-SNAPSHOT-linux-ppc64le.tar.gz   -Dbuild.snapshot=false  || ret=$?
if [ $ret -ne 0 ]; then
	set +ex
