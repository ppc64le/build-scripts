...
# Maintainer : Pratik Pahanpate <pratik.pahanpate@ibm.com>
#
...
echo "Updated CMAKE_PREFIX_PATH after Protobuf: $CMAKE_PREFIX_PATH"
...
# Adding this source due to:
# Unable to detect linker for compiler `cc -Wl,--version`
...
python3.12 -m pip install ml-dtypes
...
export PYTHON_INCLUDE=$(python -c \
    "from sysconfig import get_paths as gp; print(gp()['include'])")

export PYTHON_LIB=$(python -c \
...
if !(python3.12 -m pip install .); then
    ...
    echo "------------------$PACKAGE_NAME: ONNX Install Failed---------------------"

    echo "$PACKAGE_URL $PACKAGE_NAME"

    echo "$PACKAGE_NAME | ONNX | v1.19.0 | GitHub | Fail | Install_Fails"
