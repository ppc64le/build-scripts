...
python3.14 -m pip wheel -v . --no-build-isolation --no-deps --config-setting="--plat-name=linux_ppc64le" -w "$CURRENT_DIR/" || \
python3.14 setup.py bdist_wheel --plat-name linux_ppc64le --dist-dir "$CURRENT_DIR/"
