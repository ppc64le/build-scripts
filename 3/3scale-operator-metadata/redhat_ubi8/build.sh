# ----------------------------------------------------------------------------
# Package       : 3scale-operator-metadata
# Version       : v0.5.1
# Source repo   : https://github.com/3scale/3scale-operator-metadata
# Tested on     : RHEL_8.3
...
# clone branch/release passed as argument, if none, use last release: v0.5.1
if [ -z $1 ] || [ "$1" == "lasttestedrelease" ]
then
	# As of 30-Apr-2021, this was tagged as 'latest_release'
	BRANCH="--branch v0.5.1"
else
	BRANCH="--branch $1"
fi
...
(git clone $BRANCH https://github.com/3scale/3scale-operator-metadata) || (echo "git clone failed"; exit $?)
cd 3scale-operator-metadata
...
# cleanup
cd ..
rm -rf 3scale-operator-metadata
