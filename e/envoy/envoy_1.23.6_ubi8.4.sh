...
if [ -z "$(ls -A $wdir/bazel)" ]; then
        ...
        git apply ../bazel-${BAZEL_VERSION}.patch
        ./compile.sh
fi
export PATH=$PATH:$wdir/bazel/output

#Build and setup clang
cd $wdir
wget https://github.com/llvm/llvm-project/releases/download/llvmorg-14.0.1/clang+llvm-14.0.1-powerpc64le-linux-rhel-8.4.tar.xz
tar -xvf clang+llvm-14.0.1-powerpc64le-linux-rhel-8.4.tar.xz

#Build Envoy
cd $wdir/${PACKAGE_NAME}
git apply ../${PACKAGE_NAME}_${PACKAGE_VERSION}.patch
bazel/setup_clang.sh $wdir/clang+llvm-14.0.1-powerpc64le-linux-rhel-8.4/

export PATH=$wdir/clang+llvm-14.0.1-powerpc64le-linux-rhel-8.4/bin:$PATH
echo "build --config=clang" >> user.bazelrc
bazel build -c opt envoy --cxxopt=-fpermissive
export ENVOY_BIN=$wdir/envoy/bazel-bin/source/exe/envoy-static
