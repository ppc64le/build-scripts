...
SCRIPT_PACKAGE_VERSION=v8.19.13
...
# Build
./gradlew :distribution:archives:linux-ppc64le-tar:assemble --parallel -x compileMain25Java   -x compileMain26Java
...
#./gradlew test -x :x-pack:plugin:ml:test -x :x-pack:plugin:esql:test --no-parallel -x compileMain25Java   -x compileMain26 Java -Dtests.haltonfailure=false
