...
# Uncomment below lines for creating image streams in Openshift 4.3
...
#podman tag quay.io/eclipse/che-plugin-registry:7.13.1-alpine $HOST/local-images/che-plugin-registry:7.13.1-alpine
#podman tag quay.io/eclipse/che-plugin-registry:7.13.1-rhel $HOST/local-images/che-plugin-registry:7.13.1-rhel

#podman login -u kubeadm -p $(oc whoami -t) --tls-verify=false $HOST
#podman push $HOST/local-images/che-plugin-registry:7.13.1-alpine --tls-verify=false
#podman push $HOST/local-images/che-plugin-registry:7.13.1-rhel --tls-verify=false
