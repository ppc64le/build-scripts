...
yum install -y wget python3.12 python3.12-pip python3.12-devel  gcc-toolset-15 gcc-toolset-15-binutils gcc-toolset-15-binutils-devel gcc-toolset-15-gcc-c++ git make cmake binutils pkgconfig 
...
python3.12 -m pip install numpy==2.2.6 setuptools==77.0.1
...
if ! (python3.12 -m pytest --pyargs h5py -k "not test_append_permissions and not test_complex256 and not test_long_double and not test_custom_float_promotion"); then
