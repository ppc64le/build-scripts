...
# Maintainer    : Sandeep Gupta <Sandeep.Gupta12@ibm.com>
#
...
yum install -y openssl-devel unzip libzip-devel.ppc64le gzip.ppc64le python3.11-devel python3.11-pip cmake
...
python3.11 -m pip install Cython==0.29.36 numpy==1.26.4 pkgconfig pytest-mpi setuptools==78.0.1
python3.11 -m pip install wheel pytest  pytest-mpi tox

export SITE_PACKAGE_PATH=/usr/local/lib/python3.11/site-packages
...
if ! (python3.11 setup.py build);then
