...
# Version			: 3.6.1
# Source repo		: http://gitbox.apache.org/repos/asf/commons-math
# Tested on			: RHEL 7.6
...
# variables
PKG_NAME="commons-math"
PKG_VERSION=3.6.1
PKG_VERSION_LATEST=3.6.1
LOGS_DIRECTORY=/logs
LOCAL_DIRECTORY=/root
REPOSITORY="http://gitbox.apache.org/repos/asf/commons-math.git"

# clone, build and test specified version
cd $LOCAL_DIRECTORY
git clone $REPOSITORY $PKG_NAME-$PKG_VERSION
cd $PKG_NAME-$PKG_VERSION/
git checkout -b $PKG_VERSION tags/MATH_3_6_1
mvn install | tee $LOGS_DIRECTORY/$PKG_NAME-$PKG_VERSION.txt

# clone, build and test latest version
#cd $LOCAL_DIRECTORY
#git clone $REPOSITORY $PKG_NAME-$PKG_VERSION_LATEST-new
#cd $PKG_NAME-$PKG_VERSION_LATEST-new/
#git checkout -b new$PKG_VERSION_LATEST tags/MATH_3_6_1
#mvn install | tee $LOGS_DIRECTORY/$PKG_NAME-$PKG_VERSION_LATEST-new.txt
