...
# ----------------------------------------------------------------------------
set -e
PACKAGE_NAME=nimbus-jose-jwt
PACKAGE_URL=https://bitbucket.org/connect2id/nimbus-jose-jwt.git
PACKAGE_VERSION=${1:-9.37.3}
...
#Build
if ! mvn -B clean install -Dgpg.skip -Dmaven.javadoc.skip=true -X ; then
