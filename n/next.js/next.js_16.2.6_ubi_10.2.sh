...
echo "Γ£ô rspack binary found: $RSPACK_BINARY"
...
echo "Γ£ô Turborepo built successfully"
...
pnpm swc-build-native || {
    echo "First pass failed ΓÇö patching wasmer traphandlers.rs and retrying..."
    ...
    if [ -n "$TRAP_FILE" ]; then
        ...
        sed -i \
            ...
            "$TRAP_FILE"
        echo "Wasmer patched ΓÇö retrying build..."
        pnpm swc-build-native
    else
...
if [ -n "$JEST_RUN" ]; then
    ...
    echo "  WARNING: jest-circus run.js not found ΓÇö skipping patch"
fi
