...
sed -i 's|^export PYTEST_ADDOPTS.*|export PYTEST_ADDOPTS := --deselect=tests/test_imports.py::test_import_time --deselect=tests/test_client_session.py|' Makefile
make
...
if ! pytest --deselect tests/test_imports.py -k "not test_client_session_timeout_zero and not test_no_warnings and not test_expires and not test_max_age and not test_cookie_jar_clear_expired and not test_c_parser_loaded and not test_invalid_character and not test_invalid_linebreak and not test_subapp and not test_middleware_subapp and not test_unsupported_upgrade and not test_get_extra_info and not test_aiohttp_plugin and not test_import_time and not test_imports and not test_simple_subapp and not test_request_tracing_url_params" --disable-warnings; then
