...
# If the active Python (set by wrapper) is missing _curses, rebuild it now
# that ncurses-devel is present. The wrapper builds Python from source using
# only ncurses (runtime) so _curses is absent; we rebuild with ncurses-devel.
if ! python -c "import _curses" 2>/dev/null; then
    PY_PATCH=$(python -c "import sys; print(f'{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}')")
