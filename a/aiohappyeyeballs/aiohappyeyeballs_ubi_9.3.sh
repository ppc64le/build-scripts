...
# Version          : v2.4.6
# Source repo      : https://github.com/aio-libs/aiohappyeyeballs
# Tested on     : UBI:9.3
# Language      : Python
# Travis-Check  : True
...
PACKAGE_VERSION=${1:-v2.4.6}
PACKAGE_URL=https://github.com/aio-libs/aiohappyeyeballs
PACKAGE_DIR=aiohappyeyeballs

# Install dependencies
yum install -y git python3 python3-devel.ppc64le gcc-toolset-13 make wget sudo cmake gcc openssl-devel
pip3 install pytest setuptools poetry
...
if ls */test_*.py > /dev/null 2>&1 && [ $test_status -ne 0 ]; then
    echo "Running pytest..."
    (poetry install) && (poetry run pytest) && test_status=0 || test_status=$?
fi
