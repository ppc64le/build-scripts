...
yum install -y --allowerasing wget git cmake ninja-build make gcc gcc-c++ gcc-toolset-12-gcc gcc-toolset-12-gcc-c++ \
    openssl openssl-devel libcurl libcurl-devel libxml2 libxml2-devel krb5-devel zlib zlib-devel pkg-config perl python39 python39-pip ca-certificates
...
# (no "cpp" suffix in the monorepo tag name)
rm -rf "$PACKAGE_DIR"
...
# Make azure-core visible to ldd for linkage validation
ldconfig /usr/local/lib64
...
python3.9 -m venv /tmp/azure-identity-test-env
