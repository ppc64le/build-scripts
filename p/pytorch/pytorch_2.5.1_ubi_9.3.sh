...
export LD_LIBRARY_PATH="$LIBPROTO_INSTALL/lib64:$ABSEIL_CPP/lib:$LD_LIBRARY_PATH"
export LD_LIBRARY_PATH="$LIBPROTO_INSTALL:${LD_LIBRARY_PATH}"
cd $SCRIPT_DIR
...
export LD_LIBRARY_PATH=/protobuf/local/libprotobuf/lib64/libprotobuf.so.25.3.0:$LD_LIBRARY_PATH
export LD_LIBRARY_PATH=/protobuf/build/libprotobuf.so.25.3.0:$LD_LIBRARY_PATH
...
if ! (pytest pytorch/test/test_utils.py -k "not test_device_mode_ops_sparse_mm_reduce_cpu_bfloat16 and not test_device_mode_ops_sparse_mm_reduce_cpu_float16  and not test_device_mode_ops_sparse_mm_reduce_cpu_float32 and not test_device_mode_ops_sparse_mm_reduce_cpu_float64 and not test_bottleneck_cpu_only"); then
