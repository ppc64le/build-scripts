...
#
# ----------------------------------------------------------------------------
#Run the script:./pgx_ubi_8.5.sh v4.6.0(version_to_test)
PACKAGE_NAME=pgx
...
ip_of_pg_server=$(docker inspect -f "{{ .NetworkSettings.IPAddress }}" postgres-server)
...
if ! PGX_TEST_DATABASE="host=$ip_of_pg_server dbname=postgres user=postgres password=password" go test -v -race ./...; then
...
#One of the test case failed and with master all test passed in manual verification.
