...
runuser -l postgres -c '/usr/local/pgsql/bin/postmaster -D /usr/local/pgsql/data2 -p 5433 >logfile 2>&1' &
...
psql -U postgres -p 5433 -c 'create database pgx_test' || true
psql -U postgres pgx_test -p 5433 -c 'create domain uint64 as numeric(20,0)' || true
psql -U postgres -p 5433 -c "create user pgx_ssl SUPERUSER PASSWORD 'secret'" || true
psql -U postgres -p 5433 -c "create user pgx_md5 SUPERUSER PASSWORD 'secret'" || true
psql -U postgres -p 5433 -c "create user pgx_pw  SUPERUSER PASSWORD 'secret'" || true
psql -U postgres -p 5433 -c "create user `whoami`" || true
psql -U postgres -p 5433 -c "create user pgx_replication with replication password 'secret'" || true
