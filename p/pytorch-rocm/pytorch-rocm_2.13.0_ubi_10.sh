...
git apply "$CURRENT_DIR/pytorch_v2.13.0_rocm_cuda_guard_narrowing.patch"

# Fix FastGeluAsm explicit specializations rejected by AMD clang 23.0 in composable_kernel
# This patch is required — fail loudly if it cannot be applied
git apply --directory=third_party/composable_kernel "$CURRENT_DIR/pytorch_v2.13.0_rocm_fastgeluasm.patch"
