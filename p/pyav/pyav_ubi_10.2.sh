...
yum install -y git make cmake binutils diffutils python3.14 python3.14-pip python3.14-devel
...
for package in openblas lame opus libvpx ffmpeg pillow numpy==2.5.0; do
...
tar -xvf lame-4.0.tar.gz

cd lame-4.0

mkdir prefix
export LAME_PREFIX=$(pwd)/prefix
...
#Build package
if ! (python3.14 setup.py build_ext --inplace) ; then
...
if ! (python3.14 -m pytest \
    --ignore=tests/test_videoframe.py \
    --ignore=tests/test_timeout.py \
    --ignore=tests/test_audiofifo.py \
    --ignore=tests/test_bitstream.py \
    --ignore=tests/test_chapters.py \
    --ignore=tests/test_packet.py \
    --ignore=tests/test_seek.py \
    --ignore=tests/test_python_io.py \
    --ignore=tests/test_open.py \
    --ignore=tests/test_streams.py \
    --ignore=tests/test_subtitles.py \
    -k "not test_add_mux_stream_video \
    and not test_decoded_video_enc_params_no_flag \
    and not test_decoded_video_enc_params \
    and not test_subtitle_encode_mp4 \
    and not test_video_remux \
    and not test_attachment_stream \
    and not test_data_types \
    and not test_qmin_qmax \
    and not test_profiles \
    ...
    and not test_mov \
    and not test_decode_video_corrupt \
    ...
    and not test_av_stream_Stream \
    and not test_encoding_with_pts \
    and not test_stream_audio_resample \
    and not test_max_b_frames \
    ...
    and not test_codec_mpeg4_decoder \
    and not test_codec_mpeg4_encoder \
    and not test_codec_delay \
    and not test_codec_tag \
    and not test_decoder_extradata \
    and not test_decoder_gop_size \
    and not test_decoder_timebase \
    and not test_encoder_extradata \
    and not test_encoder_pix_fmt \
    and not test_parse \
    and not test_sky_timelapse \
    and not test_av_codec_codec_Codec \
    and not test_av_enum_EnumFlag \
    and not test_av_enum_EnumItem \
    and not test_default_options \
    and not test_encoding \
    and not test_encoding_with_unicode_filename \
    and not test_stream_index \
    and not test_writing_to_buffer \
    and not test_writing_to_buffer_broken \
    and not test_writing_to_buffer_broken_with_close \
    and not test_writing_to_file \
    and not test_writing_to_pipe_writeonly \
    ...
    and not test_field_order \
    and not test_global_quality \
    and not test_level \
    and not test_video_codec_id \
    and not test_mux_stream_without_codec_context \
    and not test_display_matrix \
    and not test_set_rate_after_add_stream \
    and not test_codec_frame_rates \
    and not test_setters_accept_avrational \
    and not test_add_mux_stream_matroska_extradata \
    and not test_remux_keeps_keyframe_with_none_dts \
    and not test_add_stream_from_template_copies_time_base") ; then
