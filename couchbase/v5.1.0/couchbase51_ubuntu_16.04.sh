...
sudo apt-get install -y wget tar git g++ make curl libssl-dev libevent-dev \
     ...
     cmake python sudo golang-go subversion cmake openjdk-8-jdk \
     openjdk-8-jre
...
mv go go-1.7.6
mv go-1.7.6 ~/.cbdepscache/exploded/ppc64le
...
mv go go-1.8.1
mv go-1.8.1 ~/.cbdepscache/exploded/ppc64le
...
mv go go-1.8.5
mv go-1.8.5 ~/.cbdepscache/exploded/ppc64le
