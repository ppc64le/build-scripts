...
# variables
PKG_NAME="javax.activation-api"
PKG_VERSION=1.2.0
LOGS_DIRECTORY=/logs
LOCAL_DIRECTORY=/root
REPOSITORY="https://github.com/javaee/activation.git"

# clone, build and test specified version
cd $LOCAL_DIRECTORY
git clone $REPOSITORY $PKG_NAME-$PKG_VERSION
cd $PKG_NAME-$PKG_VERSION/
git checkout -b $PKG_VERSION tags/JAF-1_2_0
mvn -f activationapi/pom.xml clean install | tee $LOGS_DIRECTORY/$PKG_NAME-$PKG_VERSION.txt

# clone, build and test master
#cd $LOCAL_DIRECTORY
#git clone $REPOSITORY $PKG_NAME-master
#cd $PKG_NAME-master/
#mvn -f activationapi/pom.xml clean install | tee $LOGS_DIRECTORY/$PKG_NAME.txt
