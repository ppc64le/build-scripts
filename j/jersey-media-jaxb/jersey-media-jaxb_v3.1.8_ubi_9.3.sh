...
# -------------------------------
# Run Tests for the media/jaxb Module
# -------------------------------
cd "${MODULE_NAME}"
mvn test || ret=$?
if [ $ret -ne 0 ]; then
...
if [[ "$ACTUAL_VERSION" == "$PACKAGE_VERSION" ]]; then
    echo "PASS: ${PACKAGE_NAME}-media-jaxb version $PACKAGE_VERSION built successfully."
    exit 0
else
