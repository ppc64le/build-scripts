...
# -------------------------------
# Run Tests for the jersey-media-sse Module
# -------------------------------
# -------------------------------
cd "${MODULE_NAME}"
mvn test || ret=$?
...
if [[ "$ACTUAL_VERSION" == "$PACKAGE_VERSION" ]]; then
    echo "PASS: ${PACKAGE_NAME}-media-sse version $PACKAGE_VERSION built successfully."
    exit 0
else
