...
#Build docker image
docker buildx build --load --build-arg CASSANDRA_VERSION=4.1.10 --tag cr.k8ssandra.io/k8ssandra/cass-management-api:4.1.10 --file cassandra/Dockerfile-4.1 --platform linux/ppc64le .

#Install netty tcnative
cd $wdir && git clone https://github.com/netty/netty-tcnative.git && cd netty-tcnative && git checkout netty-tcnative-parent-2.0.74.Final
...
#Install netty
cd $wdir && git clone https://github.com/netty/netty.git && cd netty && git checkout netty-4.1.128.Final
...
sed -i "s#<module>boringssl-static</module>##g" pom.xml
mvn install -DskipTests
mkdir -p $wdir/management-api-for-apache-cassandra/management-api-server/.cassandra-bin/apache-cassandra-4.0.19/lib/
cp $wdir/netty58/netty-tcnative/openssl-static/target/netty-tcnative-openssl-static-2.0.63.Final.jar $wdir/management-api-for-apache-cassandra/management-api-server/.cassandra-bin/apache-cassandra-4.0.19/lib/netty-tcnative-boringssl-static-2.0.36.Final.jar
...
cd ../common && mvn install -DskipTests
cd ../all && mvn install -DskipTests
cp $wdir/netty58/netty/all/target/netty-all-4.1.58.Final.jar $wdir/management-api-for-apache-cassandra/management-api-server/.cassandra-bin/apache-cassandra-4.0.19/lib/netty-all-4.1.58.Final.jar

#Unit and integration tests
cd $wdir/$PACKAGE_NAME
docker tag cr.k8ssandra.io/k8ssandra/cass-management-api:4.1.10 mgmtapi-dockerfile-4.1-test:latest
...
#Run the image
#docker run --privileged -e USE_MGMT_API=true -p 8080:8080 -it --rm cr.k8ssandra.io/k8ssandra/cass-management-api:4.1.10

#Conclude
set +ex
echo "Build and tests Successful!"
echo "Image tag: cr.k8ssandra.io/k8ssandra/cass-management-api:4.1.10"
