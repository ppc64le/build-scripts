...
#
# ----------------------------------------------------------------------------
# We have disabled travis check as it takes more than 2 hours to build.
PACKAGE_NAME=ray
...
#bazel test $(bazel query 'kind(cc_test, ...)') --cxxopt='-Wno-error=maybe-uninitialized' --define=USE_OPENSSL=1|| true

#These test cases pass successfully when run separately.
#bazel test //cpp:simple_kv_store --cxxopt='-Wno-error=maybe-uninitialized' --define=USE_OPENSSL=1|| true
#bazel test //cpp:cluster_mode_xlang_test --cxxopt='-Wno-error=maybe-uninitialized' --define=USE_OPENSSL=1|| true
#bazel test //cpp:metric_example  --cxxopt='-Wno-error=maybe-uninitialized' --define=USE_OPENSSL=1|| true