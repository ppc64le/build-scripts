...
PACKAGE_NAME=redis
SCRIPT_PACKAGE_VERSION=8.4.2
PACKAGE_VERSION=${1:-$SCRIPT_PACKAGE_VERSION}
PACKAGE_URL=https://github.com/redis/redis.git

# Using Bitnami 8.6 structure as 8.4.2 Bitnami variant is not available
BITNAMI_VERSION=8.6.3
BITNAMI_COMMIT=${BITNAMI_COMMIT:-c5bdf5e}
...
# NOTE: Using Bitnami 8.6 structure as 8.4.2 Bitnami variant is not available
# ----------------------------------------------------------------------------
git clone https://github.com/bitnami/containers "$BUILD_HOME/containers"
cd "$BUILD_HOME/containers"
git checkout "$BITNAMI_COMMIT"

cd "$BUILD_HOME/containers/bitnami/redis/8.6/debian-12"
wget "https://downloads.bitnami.com/files/stacksmith/redis-${BITNAMI_VERSION}-0-linux-amd64-debian-12.tar.gz"
tar -xvf "redis-${BITNAMI_VERSION}-0-linux-amd64-debian-12.tar.gz"
mkdir -p prebuildfs/opt/bitnami/redis/etc
# Note: The extracted directory name doesn't include the '-0-' part
cp "redis-${BITNAMI_VERSION}-linux-amd64-debian-12/files/redis/etc/redis-default.conf" \
