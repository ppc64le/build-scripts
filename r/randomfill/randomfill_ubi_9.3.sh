...
# Package	    : randomfill
# Version	    : v1.0.4
# Source repo	    : https://github.com/browserify/randomfill
# Tested on	    : UBI 9.3
# Language          : Node
# Ci-Check      : True
...
PACKAGE_NAME=randomfill
PACKAGE_VERSION=${1:-v1.0.4}
PACKAGE_URL=https://github.com/browserify/randomfill
...
if ! npm install standard tape tap-spec; then
    echo "------------------$PACKAGE_NAME:install_fails-------------------------------------"
    echo "$PACKAGE_URL $PACKAGE_NAME"
    echo "$PACKAGE_NAME  |  $PACKAGE_URL | $PACKAGE_VERSION | GitHub | Fail |  Install_Fails"
    exit 1
