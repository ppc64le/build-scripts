# ----------------------------------------------------------------------------
#
# Package	: Appsody/init-controller docker image
...
cp package/appsody-controller /usr/bin/appsody-controller

APPSODY_CONTROLLER_BINARY=package/appsody-controller
if [ -f "$APPSODY_CONTROLLER_BINARY" ]; then
    echo "Successfully built appsody controller !"
	# Build init-controller docker image locally
	docker build -t $DOCKER_ORG/init-controller:$APPSODY_CONTROLLER_VERSION -t $DOCKER_ORG/init-controller:latest .
else 
