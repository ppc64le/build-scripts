...
yum install -y unzip wget zip git gcc-c++ zlib-devel openssl-devel libffi-devel sqlite-devel xz-devel perl ninja-build clang gfortran bzip2-devel xz lld libjpeg-turbo-devel
...
pip3 install six setuptools wheel jupyterlab Pillow pandas conan==1.62.0 plotly scipy
python3 -m pip install testpath pytest ipywidgets 'numpy<2.0'

conan install . || true && sed -i 's/ autotools.configure()/ autotools.configure(args=["--build=powerpc64le-linux-gnu"])/g' /$wdir/root/.conan/data/yasm/1.3.0/_/_/export/conanfile.py
...
python3 setup.py bdist_wheel --no-widget

#wheel will be generated in dist directory
#python3 -m pip install <path-to-wheel>
python3 -m pip install /catboost/catboost/python-package/dist/catboost-1.2.5-cp311-cp311-linux_ppc64le.whl
