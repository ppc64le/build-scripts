...
# Build postgres containers image
cd $wdir
...
docker buildx build --platform ${PLATFORM} --tag cloudnative-pg/cloudnative-pg:${VERSION} --load .
# Perform unit test
make test || true
...
#pull kind/node image for e2e tests (for power)
...
#Build fluentd-kubernetes image
# cd $wdir
