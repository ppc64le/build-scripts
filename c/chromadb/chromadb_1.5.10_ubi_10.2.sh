...
python3.13 -m pip install --upgrade maturin cffi patchelf setuptools wheel build
...
python3.13 -m pip install -r requirements.txt --prefer-binary --extra-index-url https://wheels.developerfirst.ibm.com/ppc64le/linux

cargo update generator
# Build and install chromadb
if ! python3.13 -m pip install .; then
...
ln -s $WORKDIR/pysqlite3/build/lib.linux-ppc64le-cpython-313/pysqlite3/_sqlite3.cpython-313-powerpc64le-linux-gnu.so /usr/lib64/libsqlite3.so.0

python3.13 -c "import chromadb; print(chromadb.__version__)"
if [ $? == 0 ]; then
