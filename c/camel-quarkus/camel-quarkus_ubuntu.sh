...
# -----------------------------------------------------------------------------
#
# Package       : apache/camel-quarkus
# Version       : 3.0.0-MAIN-SNAPSHOT
...
export VERSION_SUFFIX = "-${env.BRANCH_NAME.toUpperCase().replace('_','-')}-SNAPSHOT"

git clone --depth 1 --branch main https://github.com/quarkusio/quarkus.git /tmp/quarkus
./mvnw ${MAVEN_PARAMS} -Dquickly clean install -f /tmp/quarkus/pom.xml
export VERSION = ./mvnw ${MAVEN_PARAMS} help:evaluate -Dexpression=project.version -q -DforceStdout
export NEW_SNAPSHOT_VERSION = VERSION.replace('-SNAPSHOT', '') + VERSION_SUFFIX
sed -i "s/${VERSION}/${NEW_SNAPSHOT_VERSION}/g" $(find . -name pom.xml)

export MAVEN_PARAMS = '-B -e -ntp'

if ! ./mvnw ${MAVEN_PARAMS} -Dquickly clean install; then
