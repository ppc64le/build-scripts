# ----------------------------------------------------------------------------
#
# Package         : theia-ide/theia-apps/theia-php-docker
# Version         : master
# Source repo     : https://github.com/theia-ide/theia-apps/tree/master/theia-php-docker
# Tested on       : rhel_7.6
...
VERSION=master
IMAGE_TAG=theiaide/theia-php:latest

yum update -y
yum install git -y

git clone -b $VERSION https://github.com/theia-ide/theia-apps.git
cd theia-apps/theia-php-docker
...
#podman tag $IMAGE_TAG $HOST/local-images/theia-php:latest

#podman login -u kubeadm -p $(oc whoami -t) --tls-verify=false $HOST
#podman push $HOST/local-images/theia-php:latest --tls-verify=false
