...
yum install -y python3.14 python3.14-devel python3.14-pip gcc-toolset-15 make cmake \
  git wget tar pkgconfig autoconf automake libtool zlib-devel freetype-devel \
  gmp-devel openssl openssl-devel diffutils
...
cd $WORK_DIR
python3.14 -m pip install setuptools wheel build
# Build wheel 
python3.14 -m build --wheel --no-isolation --outdir="$WORK_DIR"
